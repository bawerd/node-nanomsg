...
Socket.prototype.bind = function (addr) {
  var eid = nn.Bind( this.binding, addr );
  if(eid < 0){
    return this.emit('error', new Error(nn.Err() +': '+this.type+' bind@' + addr));
  }
  this.bound[addr] = eid;
  return this.bound; //socket's active endpoints
}

Socket.prototype.connect = function (addr) {
  var eid = nn.Connect( this.binding, addr )
  if(eid < 0){
    return this.emit('error', new Error(nn.Err() +': '+this.type+' connect@' + addr));
  }
  this.connected[addr] = eid;
  return this.connected; //socket's active endpoints
}

Socket.prototype.shutdown = function (addr) {
  var eid;
  if (this.bound.hasOwnProperty(addr)) {
    eid = this.bound[addr]; delete this.bound[addr];
  } else if (this.connected.hasOwnProperty(addr)) {
    eid = this.connected[addr]; delete this.connected[addr];
  }

  if (nn.Shutdown(this.binding, eid) < 0) {
    this.emit('error', new Error(nn.Err()+': '+this.type+' shutdown@'+addr));
  }
  return eid;
};
