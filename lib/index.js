...
Socket.prototype._read = function (n) {
  ...
  //set up some pull streams against a push socket
  //send some msgs at a high interval rate

  //maybe something like:
  //var nano = require('..')
  //var push = nano.socket('push')
  //push.bind('ipc://backpressure')
  //var msg = 'hello from nanomsg!', count = 0
  //setInterval(broadcast, 10)
  //function broadcast(){
  //  push.send(++count+' :'+msg)
  //}

  //start that process, and some pull streams...
  //now come back here to index.js and watch the chunks get buffered on this._readableState
  console.log(this._readableState)

  //note: messages will stop flowing at the highwatermark
  //and meanwhile back on the push socket side something interesting to observe:
  //the message sent count is consistent with the highmark totals of all the pull sockets

  //at that point the push socket is waiting to release its internal queue to any available pull socket
  //this is the essence of the backpressure mechanism

  //i think all we need to do now is check that the flowing has stopped when the buffer is full
  //if (!this._readableState.flowing)
  //and perhaps unshift the buffer, somehow process the buffer and give it kick again..
  //so we can resume the flowing _readableState
...
Socket.prototype._write = function(buf, _, next){
    this.send(buf);
    next();
}
