...
Socket.prototype.bind = function (addr) {
  ...
  this.how[addr] = [eid,'bind'];
  return this.how; //socket's active endpoints
}
...
Socket.prototype.connect = function (addr) {
  ...
  this.how[addr] = [eid,'connect'];
  return this.how; //socket's active endpoints
}

Socket.prototype.shutdown = function (addr) {
  var confirm = this.how[addr][1] + ' endpoint '+ addr + ' shutdown';
  if(nn.Shutdown(this.binding, this.how[addr][0]) < 0){
    return this.emit('error', new Error(nn.Err()+': '+this.type+' shutdown@'+addr));
  }
